class oi3-activiti-rdbms::service {
	...
	exec { "oi3-create-activiti-db-and-schema":
                unless => "/opt/openinfinity/3.0.0/mariadb/bin/mysql -uroot -p${mysql_password} activiti",
                command => "/opt/openinfinity/3.0.0/mariadb/bin/mysql -uroot -p${mysql_password} -e \"create database activiti; grant all privileges on activiti.* to
