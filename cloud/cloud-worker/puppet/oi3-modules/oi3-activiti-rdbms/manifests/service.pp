class oi3-activiti-rdbms::service {
	...
	exec { "oi3-create-activiti-db-and-schema":
                ...
                command => "/usr/bin/mysql -uroot -p${mysql_password} -e \"create database activiti; grant all privileges on activiti.* to
                        'activiti'@'%' identified by '${activiti_password}'; flush privileges; use activiti;  source /opt/openinfinity/3.0.0/activiti/dbschema/activiti.mysql.create.engine.sql; source /opt/openinfinity/3.0.0/activiti/dbschema/activiti.mysql.create.history.sql; source /opt/openinfinity/3.0.0/activiti/dbschema/activiti.mysql.create.identity.sql; source /opt/openinfinity/3.0.0/activiti/dbschema/activiti.mysql.add.oiuser.sql\"",
